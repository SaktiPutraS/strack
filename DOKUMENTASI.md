...
## Sesi 2026-08-21 (Sinkron folder kerja lokal dengan daftar proyek strack)

Permintaan user: pastikan `D:\Project Saktify` hanya berisi pekerjaan yang MASIH JALAN. Yang proyeknya
sudah tutup di-RAR lalu dipindah ke `E:\Backup Joki`, dan kalau ada proyek jalan yang belum punya folder,
kabari.

KEPUTUSAN user:
1. Wajib punya folder = status PROGRESS + WAITING + LEAD. FINISHED + CANCELLED = boleh diarsipkan.
2. Setelah di-RAR dan arsipnya diverifikasi, folder asli DIHAPUS.
3. Dijalankan biasa = LAPORAN SAJA. `-Arsipkan` untuk mengeksekusi, dan tiap folder dikonfirmasi satu per satu.
4. strack TIDAK menyimpan path folder apa pun (permintaan user). Pemetaan folder ke proyek disimpan
   lokal di `D:\Project Saktify\_peta-folder-proyek.json`.

### File baru
- `app/Console/Commands/ProyekDaftar.php`: `php artisan proyek:daftar --json [--status=A,B]`.
  Keluaran JSON (id, klien, judul, status, deadline). Read-only, cuma untuk dibaca skrip.
- `scripts/sinkron-folder-proyek.ps1`: skrip LOKAL (dijalankan di PC, bukan di hosting).
  Ambil daftar proyek lewat `ssh saktify`, baca isi folder kerja, cocokkan, lapor, lalu (opsional) arsipkan.

### Cara kerja pencocokan
Nama folder dipecah jadi kata, dicocokkan dengan kata di nama klien (bobot 3) dan judul proyek (bobot 2),
proyek yang masih jalan dapat tambahan 0.5 sebagai pemecah seri. Kata dianggap sama bila identik atau
salah satu awalan yang lain ("analisi" ~ "analisis"). Ambang kandidat 2, ditampilkan 5 teratas.
BOBOT JUDUL SEMPAT 1 DAN AMBANGNYA 3: `Project_Starvvo` jadi tidak ketemu karena namanya HANYA judul
tanpa nama klien. Jangan dinaikkan lagi tanpa menguji kasus itu.
Hasil pilihan user disimpan di peta supaya tidak ditanya ulang. Folder bukan proyek (mis. `Saktify_Assistant`)
bisa ditandai abaikan selamanya.

### Pagar keamanan sebelum menghapus folder
Berlapis, dan folder asli TIDAK disentuh kalau salah satu gagal:
1. `rar a` harus keluar dengan kode 0 dan file arsipnya ada.
2. `rar t` (uji integritas) harus lolos.
3. Jumlah entri di dalam arsip (`rar lb -r`) tidak boleh KURANG dari jumlah berkas di folder asli.
   UKURAN FILE SENGAJA TIDAK DIPAKAI sebagai patokan: versi pertama menolak arsip < 1 KB, padahal arsip
   sah dari folder kecil memang bisa 434 byte. Jangan kembalikan pengecekan ukuran.
4. Folder KOSONG tidak diarsipkan sama sekali, hanya dilaporkan.
5. Nama arsip yang sudah ada TIDAK PERNAH ditimpa, diberi akhiran tanggal lalu tanggal+jam.

### Catatan penting
- Arsip dibuat dari `Push-Location $ProjectDir` lalu `rar a -r -- target.rar NamaFolder`, jadi isi arsip
  memuat folder induknya (`Project_X\...`), sama seperti hasil klik kanan WinRAR dan 103 arsip lama.
- Entri peta yang foldernya sudah hilang TIDAK dihitung "punya folder". Jadi kalau proyek yang sudah
  diarsipkan dibuka lagi jadi PROGRESS, dia tetap muncul di daftar "belum punya folder".
- `-DaftarFile <json>` memakai daftar proyek dari file, bukan SSH. Dipakai untuk uji.
- `-TanpaTanya` melewati seluruh tanya jawab (buat dijadwalkan). Digabung dengan `-Arsipkan` berarti
  mengarsipkan tanpa konfirmasi per folder, jadi pakai hanya kalau petanya sudah dipercaya.

### Verifikasi
- Laporan pada data ASLI: 219 proyek terbaca, 12 folder. Setelah bobot judul diperbaiki, SEMUA 11 folder
  proyek punya pasangan yang benar sebagai kandidat teratas. Tiga yang perlu dipilih sendiri karena seri
  atau tidak jelas: `Project_Cia_Revisi_TA` (#213 vs #154 sama-sama 5), `Project_Vivi_Jurnal` (5 proyek
  Vivi seri di 3), dan `Saktify_Assistant` (bukan proyek, tandai abaikan).
  Temuan sampingan: `Project_SMG_Item_Gantung` ternyata #222 PT Sinai Maju Gemilang, SMG itu singkatannya.
- Uji pengarsipan di folder percobaan (BUKAN data asli): folder proyek selesai ter-RAR + isinya benar
  (`Project_Uji_Selesai\catatan.txt`) + folder asli terhapus; folder proyek aktif tidak disentuh; folder
  kosong dilewati; proyek yang dibuka lagi setelah diarsipkan kembali muncul sebagai "belum punya folder";
  entri peta yatim dilaporkan. Semua folder uji sudah dibersihkan.
- Sintaks PowerShell diperiksa lewat `[Parser]::ParseFile`, lint PHP bersih.
- BELUM dipakai pada data asli untuk mengarsipkan apa pun. Pemetaan 12 folder menunggu user.
