...
@section('content')
            ...
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-start align-items-lg-center gap-3">
                <div>
                    <h1 class="h2 fw-bold text-purple mb-1">
                        <i class="bi bi-pencil-square me-2"></i>Edit Perlengkapan
                    </h1>
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-xl-8">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control form-control-lg @error('name') is-invalid @enderror" id="name"
                                    name="name" value="{{ old('name', $supply->name) }}" placeholder="Contoh: Kertas A4, Tinta Printer, dll" required>
                                @error('name')
                            ...
                            <div class="col-md-6">
                                <label for="qty" class="form-label fw-semibold">
                                    Jumlah/Stok Saat Ini <span class="text-danger">*</span>
                                </label>
                                <div class="input-group input-group-lg">
                                    <span
                                        class="input-group-text bg-{{ $supply->stock_status_color }} bg-opacity-10 text-{{ $supply->stock_status_color }} fw-bold">
                                        <i class="bi bi-boxes me-1"></i>
                                    </span>
                                    <input type="number" class="form-control @error('qty') is-invalid @enderror" id="qty" name="qty"
                                        value="{{ old('qty', $supply->qty) }}" min="0" placeholder="0" required>
                                </div>
                                @error('qty')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">
                                    ...
                                    Stok sebelumnya: <strong>{{ $supply->qty }}</strong>
                                </small>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group input-group-lg">
                                    ...
                                    <input type="number" class="form-control @error('minimum_stock') is-invalid @enderror" id="minimum_stock"
                                        name="minimum_stock" value="{{ old('minimum_stock', $supply->minimum_stock) }}" min="0" placeholder="0"
                                        required>
                                </div>
                                @error('minimum_stock')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-12">
                                ...
                                <div class="input-group input-group-lg">
                                    ...
                                    <input type="url" class="form-control @error('order_link') is-invalid @enderror" id="order_link" name="order_link"
                                        value="{{ old('order_link', $supply->order_link) }}" placeholder="https://...">
                                </div>
                                @error('order_link')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="4"
                                    placeholder="Catatan tambahan tentang barang ini...">{{ old('notes', $supply->notes) }}</textarea>
                                @error('notes')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center gap-3">
                            <button type="button" class="btn btn-outline-danger btn-lg" onclick="confirmDelete()">
                                <i class="bi bi-trash me-2"></i>Hapus Perlengkapan
                            </button>
                            <div class="d-flex flex-column flex-sm-row gap-2">
                                <a href="{{ route('supplies.show', $supply) }}" class="btn btn-secondary btn-lg">
                                    <i class="bi bi-x-circle me-2"></i>Batal
                                </a>
                                <button type="submit" class="btn btn-primary btn-lg">
                                    <i class="bi bi-check-circle me-2"></i>Update Perlengkapan
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <!-- Delete Form -->
            <form id="delete-form" action="{{ route('supplies.destroy', $supply) }}" method="POST" style="display: none;">
                @csrf
                @method('DELETE')
            </form>
        </div>
...
@push('scripts')
    <script>
        function confirmDelete() {
            Swal.fire({
                title: 'Yakin menghapus perlengkapan?',
                text: "Semua riwayat penggunaan juga akan terhapus!",
                icon: 'warning',
